...
__all__ = ['compute_roa', 'binary_cmap', 'constrained_batch_sampler', 'compute_closedloop_response',
           'get_parameter_change', 'import_from_directory', 'LyapunovNetwork', 'InvertedPendulum', 'CartPole', 'MassSpringDamper']
...
class MassSpringDamper(DeterministicFunction):
    """Mass-Spring-Damper.
    ...
    def __init__(self, mass, elastic_const, friction=0, dt=1 / 80,
                 normalization=None):
        """Initialization; see `MassSpringDamper`."""
    ...
    def linearize(self):
        ...
        A = np.array([[0, 1],
                      [-elastic_const / inertia, -friction/ inertia]],
                     dtype=config.np_dtype)
    ...
    def ode(self, state, action):
        ...
        position, velocity = tf.split(state, 2, axis=1)

        x_ddot = -(elastic_const / inertia)*position + action / inertia

        if friction > 0:
            x_ddot -= (friction / inertia)  * velocity

        state_derivative = tf.concat((velocity, x_ddot), axis=1)
